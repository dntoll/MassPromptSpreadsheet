...
- **Manual re-run for one cell:** Add an optional first argument `MP_REFRESH(token)` to control cache-busting per cell, e.g. `=PROMPT(MP_REFRESH(G2); A2; B2)`. Changing only `G2` reruns just that cell without changing prompt/input values.
...
**Manual re-run of one cell (without changing input/prompt):**

- **G3** contains: `1` (refresh token)
- **A3** contains: `Some text`
- **B1** contains: `Summarize: {0}`
- **B3** contains: `=PROMPT(MP_REFRESH(G3);A3;B1)`
- Increase **G3** to `2` to re-run only this formula.
